    ...
    <main className="min-h-screen bg-stone-100 text-stone-900">
        ...
        <div className="mx-auto max-w-7xl px-4 py-10">
          ...
          <div className="mt-6 max-w-3xl">
            ...
            <p className="mt-5 text-base leading-7 text-stone-600">
              Name your campaign, choose your filters, save the selected
              companies to campaign history and download a clean CSV.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8">
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <SummaryCard label="Selected companies" value={selectedCompanyRows.length} />
          <SummaryCard label="Customers" value={customerCount} urgent={customerCount > 0} />
          <SummaryCard label="Quoted" value={quotedCount} urgent={quotedCount > 0} />
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="rounded-xl border border-stone-300 bg-white px-4 py-2 text-sm font-bold hover:bg-stone-50"
              >
                Reset filters
              </button>
              ...
              <button
                ...
                className="rounded-xl border border-stone-300 bg-white px-4 py-2 text-sm font-bold hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Download only
              </button>
              ...
              <button
                ...
                className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-bold text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          <div className="mt-5 rounded-2xl border border-stone-200 bg-stone-50 p-4">
            ...
            <p className="mt-3 text-sm text-stone-600">
              ...
              {excludedCount === 1 ? ' is' : 's are'} currently excluded.
            </p>
            ...
            {rowsWithoutCompanyId > 0 ? (
              <p className="mt-2 text-sm font-semibold text-amber-700">
                ...
                campaign history because they are not linked to a company.
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
          ...
          {filteredRows.length > 100 ? (
            <div className="border-t border-stone-200 p-4 text-sm text-stone-500">
              Showing first 100 rows only. The CSV includes all{' '}
        ...
        <div className="absolute z-20 mt-2 max-h-72 w-full overflow-y-auto rounded-xl border border-stone-200 bg-white p-2 shadow-xl">
          {selectedValues.length > 0 ? (
            <button
              type="button"
              onClick={() => onChange([])}
              className="mb-2 w-full rounded-lg bg-stone-100 px-3 py-2 text-left text-xs font-bold text-stone-600 hover:bg-stone-200"
            >
              Clear selection
            </button>
          ) : null}
          ...
          {options.length === 0 ? (
            ...
            <div className="space-y-1">
              {options.map((option) => (
                <label
                  key={option.value}
                  className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-stone-700 hover:bg-stone-50"
                >
                  <input
                    type="checkbox"
